...
#if PARALLELISM == 1
for (i=0;i<NUM_REPETITIONS;i++) CycGrpG_copy(&PK_parallel_safe[i],PK); 
//#pragma omp parallel 
{
#pragma omp for
#endif
...
#else
if (!CycGrpG_isEqual(&GTmp,&pi->C[i][(unsigned int)Challenge[i]].PK)) { // check that g^s =PK[i][Challenge_i] 
#endif
...
if (!G2_isEqual(&T[i],&pi->C[i][(unsigned int)Challenge[i]].T)) { // check that T[i] equals the value T[i][Challenge[i]] from the proof
